...
function CaseResultsPanel({ cases, caseResults, defaultId }: {
    cases: ProblemCase[];
    caseResults: CaseResult[];
    defaultId?: string;
}) {
    const [selectedId, setSelectedId] = useState(defaultId ?? caseResults[0]?.id ?? '')
    const selected = caseResults.find(r => r.id === selectedId)

    return (
        <div>
            <div className='case-status-row'>
                {cases.map((c, i) => {
                    const r = caseResults.find(cr => cr.id === c.id)
                    const passed = r?.passed ?? false
                    const isActive = selectedId === c.id
                    return (
                        <button
                            key={c.id}
                            className={`case-badge ${passed ? 'passed' : 'failed'} ${isActive ? 'active-badge' : ''}`}
                ...
                {selected && (
                    ...
                    >
                        <div className='case-detail-section'>
                            <span className='case-detail-label'>Input</span>
                            <div className='detail-container'>{selected.input}</div>
                        </div>
                        <div className='case-detail-section'>
                            <span className='case-detail-label'>Output</span>
                            <div className='detail-container'>{selected.output}</div>
                        </div>
                        {selected.expected && (
                            <div className='case-detail-section'>
                                <span className='case-detail-label'>Expected</span>
                                <div className='detail-container'>{selected.expected}</div>
                            </div>
                        )}
                    </motion.div>
                )}
        ...
        >
            ...
            <div className='details-container'>
                <div className='last-excecuted-input-container'>
                    ...
                    <div className='detail-container'>{results.lastExecutedInput}</div>
                </div>
            </div>
        </motion.article>
    )
...
function ResultWA({ controller }: TestResultViewProps) {
    ...
    const firstFailedId = results.caseResults?.find(r => !r.passed)?.id
        ...
        >
            ...
            {results.caseResults ? (
                <CaseResultsPanel
                    ...
                    defaultId={firstFailedId}
                />
            ) : (
                <div className='details-container'>
                    <div className='individual-container-wrong-answer'>
                        Input
                        <div className='detail-container'>{results.input}</div>
                    </div>
                    <div className='individual-container-wrong-answer'>
                        Output
                        <div className='detail-container'>{results.output}</div>
                    </div>
                    <div className='individual-container-wrong-answer'>
                        Expected
                        <div className='detail-container'>{results.expected}</div>
                    </div>
                </div>
            )}
        </motion.article>
        ...
        >
            ...
            <div className='details-container-accepted'>
                <div className='individual-container-accepted'>
                    Runtime
                    <div className='detail-container-accepted'>{results.runtime} ms</div>
                </div>
                <div className='individual-container-accepted'>
                    Memory
                    <div className='detail-container-accepted'>{results.memory} MB</div>
                </div>
            </div>
            {results.caseResults && (
                <CaseResultsPanel
...
function TestResultView({ controller }: TestResultViewProps) {
    ...
    if (results.status === 'time_limit_exceeded' ||
        ...
        return <ResultOLE_TLE_MLE controller={controller} />
    }
